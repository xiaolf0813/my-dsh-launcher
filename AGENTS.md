...
5. **DSH 页面避让 CSS 依赖稳定 data 属性**：右侧栏顶条 `[data-sidebar-right-panel] [role="tablist"][data-dockkit-strip]`、会话顶栏 `[data-slot="conversation.session.header"] > header > div:first-child`，加 `padding-right: var(--dsh-launcher-inset)（144px）`。DSH 的 `wSkVaW_*`/`P3OORG_*` 等类名是 CSS modules 哈希，只能作后备选择器，升级 DSH 后需复核。
